...
int select_next_city(int num_cities, double probabilities[num_cities][num_cities], int current_city, _Bool visited[]) {
    double sum = 0.0;
    for (int i = 0; i < num_cities; i++) {
        if (!visited[i]) {
            sum += probabilities[current_city][i];
        }
    }
    ...
    for (int i = 0; i < num_cities; i++) {
        if (!visited[i]) {
            partial_sum += probabilities[current_city][i];
...
double ant_tour(int num_cities, int starting_point, double distances[num_cities][num_cities], double probabilities[num_cities][num_cities], int ant_id, int tour[]) {
    ...
    int current_city = starting_point;
    for (int i = 1; i < num_cities; i++) {
        int next_city = select_next_city(num_cities, probabilities, current_city, visited);
        tour[i] = next_city;
        visited[next_city] = 1;
        tour_distance += distances[current_city][next_city];
        current_city = next_city;
    }
    tour_distance += distances[current_city][starting_point]; // Return to starting city
...
int main() {
    ...
    // Jika file tidak ada
        ...
        int valid = 0, max_index = 0, start_index = 0;
